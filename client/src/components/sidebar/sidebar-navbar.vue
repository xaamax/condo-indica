    ...
    <div class="flex items-center">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger as-child>
          <Button variant="outline" class="border-0 flex items-center w-full justify-start">
            ...
            <span class="ml-2 hidden md:flex justify-start flex-col items-start">
              <p class="mb-0">{{ store.auth?.userData?.name }}</p>
              <small class="text-xs text-slate-400 font-light">{{ store.auth?.userData?.email }}</small>
            </span>
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent class="w-56 relative mr-4">
          <DropdownMenuLabel>{{ store.auth?.userData?.name }}</DropdownMenuLabel>
          <DropdownMenuSeparator />
          <DropdownMenuItem v-if="store.auth?.userData?.profile_current" class="cursor-default">
            <User class="mr-2 h-4 w-4" />
            <span>{{ store.auth?.userData?.profile_current }}</span>
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem @click="store.logout()" class="cursor-pointer">
            <LogOut class="mr-2 h-4 w-4" />
            <span>Sair</span>
            <DropdownMenuShortcut><Icon name="SignOut" /></DropdownMenuShortcut>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
